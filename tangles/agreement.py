...
class AgreementFunc(ABC):
    ...
    def __call__(
        self,
        sep_ids: Union[np.ndarray, list, tuple],
        orientation: Union[np.ndarray, list, tuple],
    ) -> int:
...
def memoize_agreement_func(agr_func: AgreementFunc) -> AgreementFunc:
    cache: dict[tuple[int, ...], int] = {}

    def agreement(
        sep_ids: Union[np.ndarray, list, tuple],
        orientations: Union[np.ndarray, list, tuple],
    ) -> int:
        key = tuple(sep_ids) + tuple(orientations)
        good = cache.get(key)
        if good is None:
            good = agr_func(sep_ids, orientations)
            cache[key] = good
        return good

    agreement.max_value = agr_func.max_value
    return agreement
...
class AgreementFuncBitarray(AgreementFunc):
    def __init__(self, sep_sys: SetSeparationSystem, memoized=False):
        ...
        self.agreement = (
            memoize_agreement_func(self._compute_agreement)
            if memoized
            else self._compute_agreement
        )
    ...
    def _compute_agreement(
        ...
        small_side_union = (
            self.sep_sys.seps_ba[1][sep_ids[0]]
            if orientations[0] > 0
            else self.sep_sys.seps_ba[0][sep_ids[0]]
        ).copy()
        for i in range(1, len(sep_ids)):
            small_side_union |= (
                self.sep_sys.seps_ba[1][sep_ids[i]]
                if orientations[i] > 0
                else self.sep_sys.seps_ba[0][sep_ids[i]]
            )
...
class AgreementFuncBitarrayBips(AgreementFunc):
    ...
    def _compute_agreement(
        ...
        intersection = (
            self.sep_sys.seps_ba[sep_ids[0]]
            if orientations[0] > 0
            else ~self.sep_sys.seps_ba[sep_ids[0]]
        ).copy()
        for i in range(1, len(sep_ids)):
            intersection &= (
                self.sep_sys.seps_ba[sep_ids[i]]
                if orientations[i] > 0
                else ~self.sep_sys.seps_ba[sep_ids[i]]
            )
        return intersection.count()


class AdvancedAgreementFunc(AgreementFunc):
    def __init__(self, sep_sys: AdvancedFeatureSystem, memoized=False):
        super().__init__(sep_sys.datasize)
        self.sep_sys = sep_sys
        self.agreement = (
            memoize_agreement_func(self._compute_agreement)
    ...
    def _compute_agreement(
        ...
        s = self.sep_sys.features[:, sep_ids] * orientations
        return s[np.minimum.reduce(s, axis=1) > 0, :].sum()
...
def agreement_func(sep_sys: SetSeparationSystemBase, **options) -> AgreementFunc:
    ...
    if isinstance(sep_sys, AdvancedFeatureSystem):
        return AdvancedAgreementFunc(sep_sys, **options)

    raise NotImplementedError(f"agreement_func does not support {type(sep_sys)}")


class CalculateAgreement:
    def __init__(self, agr_func: AgreementFunc, forbidden_tuple_size: int):
        self._agreement_func = agr_func
        self._forbidden_tuple_size = forbidden_tuple_size

    def _calculate_subset_agreements(
        self, subsets: np.ndarray, inclusions: np.ndarray, tail: np.ndarray
    ) -> np.ndarray:
    ...
    def __call__(self, tangles: list[Tangle], add_seps: set[OrientedSep]):
        ...
        for subset_size in range(0, self._forbidden_tuple_size - len(add_seps) + 1):
            ...
            new_core_agreement = self._calculate_subset_agreements(
