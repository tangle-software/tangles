...
class FTreeNode(BinTreeNode):
    ...
    def __init__(self, new_sep_id: int, parent=None, left_child=None, right_child=None):
    ...
    def make_irreducible(self):
        ...
        while stack:
            node = stack.pop()
            while (
                w := node._search_child_whose_leaves_do_not_require_parent_to_be_forbidden()
            ) is not None:
    ...
    def _search_child_whose_leaves_do_not_require_parent_to_be_forbidden(
        self,
    ) -> Optional["FTreeNode"]:
        if self.is_leaf():
            return None
        ...
        left_forbidden_tuples = [
            leaf.forbidden_tuples for leaf in self.left_child.leaves_in_subtree()
        ]
        right_forbidden_tuples = [
            leaf.forbidden_tuples for leaf in self.right_child.leaves_in_subtree()
        ]

        can_take_left = True
        for tuples in left_forbidden_tuples:
            if all(
                (parent_sep_id, 1) in forbidden_tuple
                or (parent_sep_id, -1) in forbidden_tuple
                for forbidden_tuple in tuples
            ):
                can_take_left = False
                break
    ...
    def remove_sep_from_leaves(self, sep_id: int):
        ...
        for leaf in leaves:
            leaf.forbidden_tuples = [
                forbidden_tuple
                for forbidden_tuple in leaf.forbidden_tuples
                if (sep_id, 1) not in forbidden_tuple
                and (sep_id, -1) not in forbidden_tuple
            ]


def _which_nodes_have_certain_subset(
    node_list: list[FTreeNode], subset_size: int
) -> dict:
...
def _create_next_level(
    level_nodes: list[FTreeNode], allowed_agreement: float, sep_id_before_new_level: int
):
    next_level_nodes = []
        ...
        if tst_node.agreement >= allowed_agreement:
            if tst_node.left_child and tst_node.right_child:
                ...
                next_level_nodes.extend(
                    [
                        (f_tree_node.left_child, tst_node.left_child),
                ...
                if tst_node.left_child.agreement < allowed_agreement:
                    forced_decreases_agreement_warning(
                        sep_id_before_new_level,
                        -1,
                        tst_node.core,
                        tst_node.agreement,
                        tst_node.left_child.agreement,
                    )
                ...
                if tst_node.right_child.agreement < allowed_agreement:
                    forced_decreases_agreement_warning(
                        ...
                        tst_node.core,
                        tst_node.agreement,
                        tst_node.right_child.agreement,
                    )
    return next_level_nodes


def forced_decreases_agreement_warning(
    sep_id_before_new_level, ori, previous_core, prev_agreement, new_agreement
):
    raise ValueError(
        f"""
          Error!
...
def _find_forbidden_tuples(
    parents: list[FTreeNode],
    oriented_sep: OrientedSep,
    ag_func: AgreementFunc,
    allowed_agreement: float,
    tuple_size: int,
):
    if tuple_size == 0:
    ...
    if tuple_size == 1:
        ...
        for subset, inclusion in _which_nodes_have_certain_subset(
            parents, subset_size=tuple_size - 1
        ).items():
            buf[1:, :] = subset
            if ag_func(buf[:, 0], buf[:, 1]) < allowed_agreement:
...
def _check_level_nodes(
    level_nodes, sep_id, allowed_agreement, agreement_func, tuple_size
):
    forbidden_nodes = [
        f_t_node
        for f_t_node, tst_node in level_nodes
        if tst_node.agreement < allowed_agreement
    ]
    ...
    for ts in range(tuple_size + 1):
        _find_forbidden_tuples(
            left_parents, (sep_id, -1), agreement_func, allowed_agreement, ts
        )
        _find_forbidden_tuples(
            right_parents, (sep_id, 1), agreement_func, allowed_agreement, ts
        )


def create_ftree(
    tst: TangleSearchTree,
    agreement_func: AgreementFunc,
    allowed_agreement: float,
    forbidden_tuple_size=3,
):
    root = FTreeNode(new_sep_id=None)
    level_nodes = [(root, tst.root)]
    for level_num, sep_id in enumerate(tst.sep_ids):
        if not (
            level_nodes := _create_next_level(level_nodes, allowed_agreement, sep_id)
        ):
            break
        _check_level_nodes(
            level_nodes,
            ...
            allowed_agreement,
            agreement_func,
            min(level_num + 1, forbidden_tuple_size),
        )
    return root
