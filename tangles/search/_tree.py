...
class TangleSearchTree:
    ...
    def max_leaf_agreement(self, check_last_level=False) -> int:
        ...
        for _, level in self._levels(
            self._root, 0, len(self._sep_ids) + (1 if check_last_level else 0)
        ):
    ...
    def search_tree(self, agreement: int = 1) -> Tangle:
        """
    ...
    def maximal_tangles(
        ...
        include_spl_levels, include_spl_nodes = (
            include_splitting == "levels",
        ...
        last_level = None
        for _, level in self._levels(
            self.root, agreement, max_level if max_level is not None else None
        ):
            last_level = level
            if include_spl_levels:
                if next(
                    (
                        n
                        for n in level
                        if len([c for c in n.children() if c.agreement >= agreement])
                        != 1
                    ),
                    0,
                ):
                ...
                nodes.extend(
                    n
                    for n in level
                    if len([c for c in n.children() if c.agreement >= agreement]) != 1
                )
            else:
                nodes.extend(
                    n
                    for n in level
                    if len([c for c in n.children() if c.agreement >= agreement]) == 0
                )
        ...
        if last_level:
            nodes.extend(
                c.left_child
                for c in last_level
                if c.left_child and c.left_child.agreement >= agreement
            )
            nodes.extend(
                c.right_child
                for c in last_level
                if c.right_child and c.right_child.agreement >= agreement
            )
        return nodes
    ...
    def tangle_matrix(
        ...
        return_nodes=False,
    ) -> Union[np.ndarray, tuple[np.ndarray, list[Tangle]]]:
        ...
        nodes = self.maximal_tangles(
            agreement,
            max_level,
            include_splitting="nodes" if include_splitting else "nope",
        )
        matrix = BinTreeNode.to_indicator_matrix(nodes)
        return (matrix, nodes) if return_nodes else matrix

    def get_efficient_distinguishers(
        self, return_ids=True, agreement: int = 1, max_level: Optional[int] = None
        ...
        level_idcs = []
        ids = []
        for i, level in self._levels(
            self._root, agreement, max_level or len(self._sep_ids)
        ):
            for node in level:
                if (
                    node.left_child is not None
                    and node.right_child is not None
                    and node.left_child.agreement >= agreement
                    and node.right_child.agreement >= agreement
                ):
                    level_idcs.append(i)
                    ids.append(self.sep_ids[i])
                    break
        return (
            (np.array(level_idcs), np.array(ids))
            if return_ids
            else np.array(level_idcs)
        )

    def k_tangles(
        self, k: int, agreement: int, include_splitting: str = "nope"
    ) -> list[Tangle]:
        """
        ...
        nodes = [
            node
            for node in self._root.level_in_subtree(k)
            if node.agreement >= agreement
        ]
        include_levels, include_nodes = (
            include_splitting == "levels",
            include_splitting == "nodes",
        )
    ...
    def level_sizes_for_agreement(self, agreement: int):
        sizes = np.zeros(len(self.sep_ids) + 1, dtype=int)
    ...
    def _levels(
        self, starting_node: Tangle, agreement: int, max_depth: Optional[int] = None
    ) -> Generator[tuple[int, list[Tangle]], None, None]:
        level = [starting_node]
        starting_level = level[0].level()
        max_depth = (
            max_depth if max_depth is not None else len(self._sep_ids) + 1
        )  # Danger: max_depth or None behaves in an unexpected way for max_depth == 0
        for i in range(starting_level, max_depth):
            level = [node for node in level if node.agreement >= agreement]
            yield i, level
            level = [node.left_child for node in level if node.left_child] + [
                node.right_child for node in level if node.right_child
            ]
            if not level:
                break
        ...
        for _, my_level in self._levels(self.root, -2):
            ...
            for my_node, node in zip(my_level, level):
                if not my_node.equal_data(node):
                    ...
                    return False
                if ((not node.left_child) and my_node.left_child) or (
                    (not node.right_child) and my_node.right_child
                ):
    ...
    def load(filename: str) -> "TangleSearchTree":
        ...
        parent_index = my_list[0]
        which_child = my_list[1]
        cores = my_list[2]
        tangle_agreements = my_list[3]
        sep_ids = my_list[4]
        nodes = []
