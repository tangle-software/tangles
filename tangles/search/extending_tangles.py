...
class ExtendingTangles:
    ...
    def extend_tangles(
        ...
        forced_orientations = self._core_logic.which_nodes_are_forced(
            level_to_extend, new_sep_id
        )
        ...
        surviving_nodes_left = self._add_seps_to_tangles(
            nodes_to_test.left_children(), {(new_sep_id, -1)}, forbidden_agreement
        )
        surviving_nodes_right = self._add_seps_to_tangles(
            nodes_to_test.right_children(), {(new_sep_id, 1)}, forbidden_agreement
        )
        self._update_descendant_cores(
            _Level(surviving_nodes_left), (new_sep_id, -1), sep_ids, forbidden_agreement
        )
        self._update_descendant_cores(
            _Level(surviving_nodes_right), (new_sep_id, 1), sep_ids, forbidden_agreement
        )

    def _update_descendant_cores(
        self,
        pending_level: "_Level",
        new_sep: OrientedSep,
        sep_ids: np.ndarray,
        forbidden_agreement: int,
    ):
        for level_sep_id in sep_ids:
            if pending_level.is_empty():
                break
            forced_side = self._core_logic.which_level_side_is_forced(
                new_sep, level_sep_id
            )
            if forced_side == 1:
                ...
                forced_nodes, nodes_to_test = self._core_logic.which_nodes_were_forced(
                    pending_level, level_sep_id
                )
                forced_nodes.copy_data_into_children()
                next_level = (
                    forced_nodes.left_children() + forced_nodes.right_children()
                )
                shadowing_side = (
                    self._core_logic.by_which_orientation_is_new_sep_shadowed(
                        new_sep, level_sep_id
                    )
                )
                if shadowing_side != 1:
                    self._core_logic.remove_shadowed_seps_using_shadowed_parent(
                        nodes_to_test.right_children()
                    )
                    next_level += self._add_seps_to_tangles(
                        nodes_to_test.right_children(),
                        {new_sep, (level_sep_id, 1)},
                        forbidden_agreement,
                    )
                if shadowing_side != -1:
                    self._core_logic.remove_shadowed_seps_using_shadowed_parent(
                        nodes_to_test.left_children()
                    )
                    next_level += self._add_seps_to_tangles(
                        nodes_to_test.left_children(),
                        {new_sep, (level_sep_id, -1)},
    ...
    def _add_seps_to_tangles(
        self,
        tangles: list[Tangle],
        add_seps: set[OrientedSep],
        forbidden_agreement: int,
    ) -> list[Tangle]:
        if len(tangles) == 0:
            return []
...
class _CoreLogic:
    ...
    def which_nodes_are_forced(
        self, level: "_Level", new_sep_id: int
    ) -> np.ndarray[int]:
        subsets, inclusions = all_subsets(
            [tangle.core for tangle in level.get_nodes()], 1
        )
    ...
    def remove_shadowed_seps(self, level: "_Level", new_sep_id: int):
        ...
        seps_not_dominated_left = {
            sep for sep in all_seps if not self._le_func(new_sep_id, -1, sep[0], sep[1])
        }
        seps_not_dominated_right = {
            sep for sep in all_seps if not self._le_func(new_sep_id, 1, sep[0], sep[1])
        }
        for node in level.get_nodes():
            node.left_child.core = node.left_child.core.intersection(
                seps_not_dominated_left
            )
            node.right_child.core = node.right_child.core.intersection(
                seps_not_dominated_right
            )

    def which_level_side_is_forced(
        self, new_sep: OrientedSep, level_sep_id: int
    ) -> Optional[int]:
    ...
    def which_nodes_were_forced(
        self, level: "_Level", level_sep_id: int
    ) -> tuple["_Level", "_Level"]:
        nodes_needing_tests = []
        ...
        for node in level.get_nodes():
            if (
                ...
                and (level_sep_id, 1) in node.right_child.core
            ):
                nodes_needing_tests.append(node)
            elif (
                node.left_child is None
                or (level_sep_id, -1) not in node.left_child.core
            ):
    ...
    def by_which_orientation_is_new_sep_shadowed(
        self, new_sep: OrientedSep, level_sep_id: int
    ) -> Optional[int]:
...
class _Level:
    ...
    def nodes_with_two_children(self) -> "_Level":
        return _Level(
            [node for node in self._nodes if node.left_child and node.right_child]
        )
