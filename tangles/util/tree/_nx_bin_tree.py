...
class BinTreeNetworkX:
    ...
    def __init__(
        ...
        self._root = next(
            ...
            if self._graph.nodes[n][BinTreeNetworkX.node_attr_bin_tree_node].parent
            is None
        )
        self._pos = None

        self.tst_layout(
            rankdata([node.level() for node in nodes], method="dense")
            if not depths_for_layout
            else depths_for_layout
        )
    ...
    def _build_graph(self, nodes: list[BinTreeNode], ids: list[int]) -> nx.DiGraph:
        ...
        nx.set_node_attributes(
            graph, dict(zip(ids, nodes)), BinTreeNetworkX.node_attr_bin_tree_node
        )
        child_side_dict = {}
        for node_id, node in zip(ids, nodes):
            ...
            if test_node:
                ...
                graph.add_edge(ids[parent_idx], node_id)
                child_side_dict[node_id] = test_node.left_child == node
        nx.set_node_attributes(
            graph, child_side_dict, BinTreeNetworkX.node_attr_is_originally_left_child
        )
        return graph
    ...
    def get_ids_from_label(self, label: str) -> list[int]:
        ...
        return [
            node_id for node_id in self._labels.keys() if self._labels[node_id] == label
        ]

    def tst_layout(
        self, depths: dict[int, float]
    ...
    @property
    def x_lim(self):
        return min(p[0] for p in self._pos.values()), max(
            p[0] for p in self._pos.values()
        )

    @property
    def y_lim(self):
        return min(p[1] for p in self._pos.values()), max(
            p[1] for p in self._pos.values()
        )

    def draw(
        self,
        ax=None,
        draw_node_label_func=None,
        draw_edge_label_func=None,
        node_label_size=0.05,
        edge_label_size=0.03,
        level_label_width=1,
        draw_levels=False,
        ...
        if draw_node_label_func is None:
            ...
            xlim = (
                ax.get_xlim()
            )  # hack to avoid moving positions out of sync later... TODO v1.1
            ...
            for node_id in G.nodes:
                ...
                node_ax = plt.axes(
                    [
                        xa - node_label_size / 2,
                        ya - node_label_size / 2,
                        node_label_size,
                        node_label_size,
                    ]
                )
        ...
        if draw_edge_label_func is not None:
            ...
            for parent_id in G.nodes:
                ...
                label_y = (
                    parent_pos[1] + max(self._pos[child_id][1] for child_id in children)
                ) / 2
                for child_id in children:
                    child_pos = self._pos[child_id]
                    label_x = parent_pos[0] + (
                        (label_y - parent_pos[1]) / (child_pos[1] - parent_pos[1])
                    ) * (child_pos[0] - parent_pos[0])
                    xa, ya = tr_axes(tr_figure((label_x, label_y)))
                    label_ax = plt.axes(
                        [
                            xa - edge_label_size / 2.0,
                            ya - edge_label_size / 2.0,
                            edge_label_size,
                            edge_label_size,
                        ]
                    )
        ...
        if draw_levels and self._pos:
            ...
            for ypos, level in sorted(
                ...
                ax.axhline(y=ypos, linestyle="--", zorder=-2, **axhline_kwargs)
