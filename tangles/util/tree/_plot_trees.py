...
def compute_tree_node_positions(
    central_tree_node: TreeNode,
    ...
    node_coords, edge_coords = np.empty((total_nodes, 2)), np.empty(
        (total_nodes - 1, 2, 2)
    )
...
def _ev_central_node(some_tree_node: TreeNode):
    ...
    adj = np.zeros((len(all_tree_nodes), len(all_tree_nodes)), dtype=np.int8)
    for n_i, n in enumerate(all_tree_nodes):
        for m in n.neighbours:
            m_i = node_idx_map[m]
            adj[n_i, m_i] = adj[m_i, n_i] = 1
    _, eigv = np.linalg.eigh(adj)
    return all_tree_nodes[eigv[:, -1].argmax()]
...
def _find_central_tree_node_bfs(some_tree_node: TreeNode):
    ...
    if path and (l := len(path)) > 2:
        n = path[l // 2]
        return m if l % 2 == 0 and (m := path[l // 2 - 1]).degree() > n.degree() else n
...
def plot_tree(
    ...
    scale = 0.5 * (xlim[1] - xlim[0] - max_node_size[0]), 0.5 * (
        ylim[1] - ylim[0] - max_node_size[1]
    )
    ...
    central_node = (
        _find_central_tree_node_bfs(some_tree_node) if search_center else some_tree_node
    )
    if central_node.degree() == 0:
        nodes, node_coords, edges, edge_coords = (
            np.array([central_node]),
            np.array([[0, 0]]),
            np.empty(0),
            np.empty(0),
        )
        ...
        nodes, node_coords, edges, edge_coords = compute_tree_node_positions(
            central_node
        )
    ...
    if plot_edge_annotation:
        for i, (n1, n2) in enumerate(edges):
            ...
            edge_annotation_ax = plt.axes(
                [
                    xa - edge_annotation_size[0] * 0.5,
                    ya - edge_annotation_size[1] * 0.5,
                    edge_annotation_size[0],
                    edge_annotation_size[1],
                ]
            )
    ...
    if plot_node is None:
        node_sizes, angles = np.ones(node_coords.shape[0]) * max_node_size[0], np.zeros(
            node_coords.shape[0]
        )
        node_circles = mc.EllipseCollection(
            node_sizes,
            node_sizes,
            angles,
            offsets=node_coords,
        ...
        for i in range(nodes.shape[0]):
            ...
            node_ax = plt.axes(
                [xa - size[0] * 0.5, ya - size[1] * 0.5, size[0], size[1]],
                projection=ax_projection,
            )
