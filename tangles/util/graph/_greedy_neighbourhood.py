...
def greedy_neighborhood_old(
    ...
    if isinstance(start_neighborhood, int):
        ...
        assert isinstance(start_neighborhood, list) or isinstance(
            start_neighborhood, np.ndarray
        )
        neighborhood = (
            start_neighborhood
            if isinstance(start_neighborhood, list)
            else start_neighborhood.tolist()
        )
    ...
    start_graph = adj[neighborhood, :]
    heap.push_all(start_graph.indices, sign * start_graph.data)
    while len(neighborhood) < size and len(heap) > 0:
        for o in [
            heap.pop() for _ in range(min(take, size - len(neighborhood), len(heap)))
        ]:
            neighborhood.append(o)
            heap.push_all(
                adj.indices[adj.indptr[o] : adj.indptr[o + 1]],
                sign * adj.data[adj.indptr[o] : adj.indptr[o + 1]],
            )
    return np.array(neighborhood)


def greedy_neighborhood(
    adj: sparse.csr_array,
    start_neighborhood: Union[int, list, np.ndarray],
    size: int,
    take: int = 5,
    ...
    neighborhood = np.zeros(adj.shape[0], dtype=bool)
    neighborhood[start_neighborhood] = True

    while (n_size := neighborhood.sum()) < size:
        boundary = adj[neighborhood, :].sum(axis=0).A1
