...
def grid_distance_graph(
    ...
    coord_neighborhood = np.stack(
        np.meshgrid(
            range(-max_dist_int, max_dist_int + 1),
            range(-max_dist_int, max_dist_int + 1),
        ),
        2,
    ).reshape(neighborhood_size * neighborhood_size, 2)

    if dist == "euklid":
        dist_neighborhood = np.sqrt(np.square(coord_neighborhood).sum(axis=1))
    elif dist == "manhattan":
        dist_neighborhood = np.abs(coord_neighborhood).sum(axis=1).astype(float)
    else:
        raise NotImplementedError(
            f"Whoops, distance metrix {dist} has yet to be implemented! Sorry!"
        )
    ...
    pixel_coords = np.stack(
        np.meshgrid(range(grid_shape[0]), range(grid_shape[1]), indexing="ij"), 2
    )
    pixel_coords = (
        pixel_coords[:, :, :, np.newaxis]
        + coord_neighborhood[:, :, np.newaxis, np.newaxis].T
    )

    valid_coords = (pixel_coords >= 0).all(axis=2) & (
        pixel_coords < np.array(grid_shape)[np.newaxis, np.newaxis, :, np.newaxis]
    ).all(axis=2)
    ...
    all_data = np.broadcast_to(
        dist_neighborhood, (all_neighbor_indices.shape[0], dist_neighborhood.shape[0])
    )
...
def image_to_similarity_graph(
    ...
    flat_image = image.reshape(
        image.shape[0] * image.shape[1], image.shape[2] if len(image.shape) == 3 else 1
    )
    neighborhood_center_colors = flat_image.repeat(
        pixel_coord_distance_graph.indptr[1:] - pixel_coord_distance_graph.indptr[:-1],
        axis=0,
    )
    color_similarities = np.power(
        np.abs(
            neighborhood_center_colors
            - flat_image[pixel_coord_distance_graph.indices, :]
        ),
        color_sim_p,
    ).sum(axis=1)
...
def cosine_similarity(
    ...
    """
    if max_neighbours is not None and max_neighbours > 0:
        neighbors_to_remove = max(0, data.shape[0] - max_neighbours)
    else:
        neighbors_to_remove = None

    if sequential and chunk_size >= data.shape[0]:
        sequential = False
    ...
    if sequential:
        data, indices, indptr = [], [], [np.array([0], dtype=int)]
        for i1 in range(0, data_normed.shape[0], chunk_size):
            i2 = min(i1 + chunk_size, data_normed.shape[0])
            sim = data_normed[i1:i2, :] @ data_normed.T
            np.fill_diagonal(sim[:, i1:], 0)
            if neighbors_to_remove:
                part = np.argpartition(sim, neighbors_to_remove, axis=1)
                for k in range(i2 - i1):
                    sim[k, part[k, :neighbors_to_remove]] = 0
...
def hamming_similarity(
    ...
    of indices where to equally sized arrays of data differ.

    Given an (n x m) matrix of data, this method calculates an (n x n) matrix where the element (i, j)
    contains the hamming distance between row i and j.
    ...
    return_sparse
        whether to return a sparse similarity matrix in csr_matrix format or
        a dense numpy array. Defaults to true, meaning that a sparse matrix is returned by default.
    sequential
        whether to calculate the matrix in one step, potentially using a lot of memory, or
        sequentially linewise, using less memory. Defaults to true, meaning that it is
        calculated linewise.

    Returns
    -------
    np.ndarray or sparse.csr_matrix
        Either a dense or a sparse matrix containing the hamming similarities
        depending on the return_sparse parameter.
    """
    if not sequential:
    ...
    sim_datatype = (
        np.uint8
        if data.shape[1] < 1 << 8
        else np.uint16 if data.shape[1] < 1 << 16 else int
    )
    ...
    sim_matrix = sparse.csr_matrix(
        (np.concatenate(sim_data), np.concatenate(indices), indptr),
...
def k_nearest_neighbors(
    X: np.ndarray, metric: str = "precomputed", k: int = 1, ties_all: bool = False
) -> sparse.csr_matrix:
    ...
    neigbhbor_idcs_func = (
        (
            lambda dist_idcs_part: np.flatnonzero(
                pw_distances[dist_idcs] <= pw_distances[dist_idcs][dist_idcs_part].max()
            )
        )
        if ties_all
        else (lambda dist_idcs_part: dist_idcs_part)
    )
    start_idx_tail = 0
...
def epsilon_neighborhood_graph(
    X: np.ndarray, max_dist: float, dist2sim=None, metric: str = "precomputed"
) -> sparse.csr_matrix:
...
def cosine_similarity_graph_csr_data(
    ...
    for i in range(mat.shape[0]):
        norm[i, 0] = np.sqrt(
            np.square(mat.data[mat.indptr[i] : mat.indptr[i + 1]]).sum()
        )
    ...
    for chunk_start in range(0, mat.shape[0], chunk_size):
        ...
        sim = np.asarray(
            ((mat[chunk_start:chunk_end, :] @ mat.T) / norm[chunk_start:chunk_end])
            / norm.T
        )
    ...
    if weight_range is not None:
        adj_matrix.data = weight_range[0] + (
            adj_matrix.data - adj_matrix.data.min()
        ) / (adj_matrix.data.max() - adj_matrix.data.min()) * (
