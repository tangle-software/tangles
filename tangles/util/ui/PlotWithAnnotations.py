...
def wrap_annotation_func_ax(annotation_func_ax: AnnotationFuncAx) -> AnnotationFuncImg:
    ...
    artists = (
        []
    )  # for some reason this version is significantly faster than any other way to clear the axes in between (that I tried ...) -> annotation_func has to return a list of the created artists
...
def plot_annotated(
    ...
    annotation_offsets,
    annotation_func: AnnotationFuncImg | AnnotationFuncTxt,
    annotation_is_image: bool = False,
    annotation_zoom: float = 1,
    label: str = None,
    title: str = None,
    xlabel: str = None,
    ylabel: str = None,
    ...
    if interactive:
        ...
        def update_annotations(dyn_annotation_x=None):
            for do in dyn_objects:
                do.remove()
            dyn_objects.clear()

            if dyn_annotation_x:
                data_y = y[(np.abs(x - dyn_annotation_x)).argmin()]
                box_x, box_y = _interpolate_box_pos(
                    dyn_annotation_x, annotation_x_positions, annotation_offsets
                )
        ...
        def mouse(event):
            if event.inaxes == ax:
                mouse_down_ptr[0] = event.name == "button_press_event" or (
                    mouse_down_ptr[0] and event.name != "button_release_event"
                )
                if mouse_down_ptr[0]:
...
def _interpolate_box_pos(dyn_annotation_x, annotation_x_positions, annotation_offsets):
    ...
    w_left, w_right = (
        1 - (dyn_annotation_x - annotation_x_positions[left_idx]) / d_total,
        1 - (annotation_x_positions[right_idx] - dyn_annotation_x) / d_total,
    )
    left_x_offset, left_y_offset = annotation_offsets[left_idx]
    right_x_offset, right_y_offset = annotation_offsets[right_idx]
    return (w_left * left_x_offset + w_right * right_x_offset), (
        w_left * left_y_offset + w_right * right_y_offset
    )
