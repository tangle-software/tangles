...
from .survey_variable import SurveyVariable, UnionOfIntervals

QuestionSelector = Callable[[SurveyVariable], bool]
RowSelector = Callable[[pd.Series], bool]
ColumnSelectionTypes = Union[
    str, int, list, np.ndarray, pd.Series, range, QuestionSelector, None
]


# TODO: rename to something like "TabularData", make Survey a subclass with special function names
class Survey:
    """
    ...
    def load(folder: str, name: str) -> "Survey":
        ...
        for v_name in var_info_df.index:
            ...
            if is_float_dtype(df.dtypes[v_name]):
                ...
                var.invalid_values = {
                    float(k): v for k, v in var.invalid_values.items()
                }
    ...
    def __init__(self, data: pd.DataFrame):
        ...
        self.variables = pd.Series(
            index=data.columns, data=[SurveyVariable(name=col) for col in data.columns]
        )
    ...
    def variable_info(
        ...
        compute_value_counts: bool = True,
    ) -> pd.DataFrame:
        ...
        df = pd.DataFrame(
            ...
            data=[
                [i] + self.variables[v].to_row() for i, v in enumerate(column_selection)
            ],
        )
    ...
    def set_pyreadstat_meta_data(
        self,
        meta,
        type_of_scale_variables: str = "numeric",
        ...
        if type_of_scale_variables and not SurveyVariable.is_valid_type_name(
            type_of_scale_variables
        ):
        ...
        for i, c in enumerate(meta.column_names):
            ...
            if variable.type == "unknown":
                if not did_warn_var_type:
                    warn(
                        "Meta data does not contain type information. We use pyreadstat's internal type instead. Please change the types later..."
                    )
            ...
            if labels:
                if erase_existing:
                    variable.set_values({}, {})
                missing_ranges = [
                    (r["lo"], r["hi"]) for r in meta.missing_ranges.get(c, [])
                ]
                valid, invalid = {}, {}
    ...
    def select_questions(
        self, column_selection: ColumnSelectionTypes, suppress_check_var_warning=False
    ) -> "Survey":
    ...
    def select_respondents(
        self,
        row_selection: Union[list, np.ndarray, range, RowSelector],
        suppress_check_var_warning=True,
    ) -> "Survey":
    ...
    def resample_respondents_to_balance_answers_of_question(
        self,
        col_name: str,
        sample_size_per_answer: int = 1000,
        value_ranges: list = None,
        suppress_check_var_warning=True,
    ):
        ...
        if var.is_numeric_type():
            if value_ranges is None:
                raise ValueError(
                    f"value_ranges have to be a list of Intervals for numeric variables, not {value_ranges}"
                )
            ...
            for r in value_ranges:
                ...
                if sel.any():
                    samples.extend(
                        np.random.choice(sel, sample_size_per_answer, replace=True)
                    )
        else:
            if value_ranges is None:
    ...
    def summarize(self, column_selection: ColumnSelectionTypes = None) -> pd.DataFrame:
        ...
        if not all(self.variables[c].is_numeric_type() for c in column_selection):
            max_num_cats = max(
                ...
                if not (v := self.variables[c]).is_numeric_type()
            )
        ...
        summary = pd.DataFrame(
            ...
            data=[
                [(v := self.variables[c]).name, v.type, v.label]
                + [np.nan] * (len(column_names) - 3)
                for c in column_selection
            ],
        )
        for c in column_selection:
            if not (var := self.variables[c]).is_nominal_type():
                summary.loc[c, ["min", "mean", "median", "max"]] = [
                    (
                        d := self.data.loc[~self.data[c].isin(var.invalid_values), c]
                    ).min(),
                    d.mean(),
                    d.median(),
                    d.max(),
            ...
            if not var.is_numeric_type():
                summary.loc[
                    c,
                    column_names[
                        min_count_col_idx : min_count_col_idx + len(var.valid_values)
                    ],
                ] = [(self.data[c] == v).sum() for v in var.valid_values.keys()]
                summary.loc[c, column_names[sum_count_col_idx]] = summary.loc[
                    c,
                    column_names[
                        min_count_col_idx : min_count_col_idx + len(var.valid_values)
                    ],
                ...
                summary.loc[c, column_names[sum_count_col_idx]] = (
                    self.data.shape[0] - self.data[c].isin(var.invalid_values).sum()
                )
    ...
    def set_variable_names(
        self,
        names: Union[list, np.ndarray, pd.Series],
        column_selection: ColumnSelectionTypes = None,
    ):
    ...
    def set_valid_and_invalid_variable_values(
        self,
        column_selection: ColumnSelectionTypes = None,
        ...
        self._check_variables_and_warn_if_needed(
            suppress_check_var_warning=suppress_check_var_warning
        )

    def replace_variable_values(
        self,
        column_selection: ColumnSelectionTypes = None,
        mapping: Union[dict, list, None] = None,
        change_type_to: Optional[str] = None,
        combine_labels_if_not_injective: bool = True,
        suppress_check_var_warning: bool = False,
    ):
        ...
        combine_labels_if_not_injective : bool
        ...
        for c in column_selection:
            ...
            # if mapping is not a dictionary, we might have to add unknown values to the lists:
            if not isinstance(mapping, dict):
                unique_values = self.data[c].unique()
                if (
                    len(
                        additional_values := set(unique_values)
                        - (
                            set(var.valid_values.keys())
                            | set(var.invalid_values.keys())
                        )
                    )
                    > 0
                ):
                    warn(
                        "Variable takes values not found in its possible values lists. We assume, they are valid values..."
                    )
                    var.add_values(
                        valid_values=zip(
                            additional_values, [str(v) for v in additional_values]
                        )
                    )
                if mapping is None:
                    mapping = dict(
                        zip(
                            var.valid_values_as_list(),
                            range(1, 1 + len(var.valid_values_as_list())),
                        )
                    )
                    mapping |= dict(
                        zip(
                            var.invalid_values_as_list(),
                            range(-len(var.invalid_values_as_list()), 0),
                        )
                    )
                elif isinstance(mapping, list):
                    mapping = dict(zip(mapping, range(1, len(mapping) + 1)))
                else:
                    raise ValueError("mapping must be a list, range or a dict")

            var.replace_values(
                mapping, combine_labels_if_not_injective=combine_labels_if_not_injective
            )
            self.data[c] = self.data[c].replace(mapping)
    ...
    def replace_variable_value_labels(
        self, mapping: dict, column_selection: ColumnSelectionTypes = None
    ):
        ...
        for v in self.variables:
            if not v.is_valid_type():
                ...
                if not is_numeric_dtype(self.data[v.name]):
                    problem_info[v.name] = "numeric variable takes non-numeric values"
                elif (
                    len(
                        np.setdiff1d(
                            self.data[v.name].unique(), v.invalid_values_as_list()
                        )
                    )
                    < 2
                ):
                ...
                if v.is_ordinal_type() and not is_numeric_dtype(self.data[v.name]):
                    problem_info[v.name] = "ordinal variable takes non-numeric values"
                elif not v.valid_values:
                    problem_info[v.name] = "list of valid values is missing"
                else:
            ...
            if verbose and not v.is_usable:
                print(
                    f"variable '{v.name}' [{v.type}] -> PROBLEM: {problem_info[v.name]}"
                )
        return problem_info
    ...
    def guess_variable_types(
        self,
        column_selection: ColumnSelectionTypes = None,
        all_integer_variables_are_ordinal=False,
        suppress_check_var_warning=True,
    ):
        ...
        warn("unfinished functionality... TODO: guess_variable_types")
        columns = self.interpret_column_selection(column_selection)
        for c in columns:
            variable = self.variables[c]
            if variable.is_valid_type():
                continue
            column = self.data[c]
            dtype_kind = column.dtype.kind
            if dtype_kind in "b":  # binary
                variable.type = "nominal"
            elif dtype_kind in "iu" or (
                dtype_kind == "f" and ((column[~column.isna()] % 1) == 0).all()
            ):
                # integer values: unfortunately, we cannot infer if the variable is ordinal, nominal or numeric
                variable.type = (
                    "ordinal" if all_integer_variables_are_ordinal else "nominal"
                )
            elif dtype_kind == "f":
    ...
    def guess_variable_value_lists(
        self,
        column_selection: ColumnSelectionTypes = None,
        invalid_value_range: Optional[
            UnionOfIntervals
        ] = UnionOfIntervals.negative_axis(),
        ...
        suppress_check_var_warning : bool
            ...
            A variable is unusable, if we can not automatically create features for the corresponding question.
        """
        for c in self.interpret_column_selection(column_selection):
            variable = self.variables[c]
            unique_values = self.data[c].unique()
            existing_labels = (
                (variable.invalid_values | variable.valid_values)
                if keep_existing_labels
                else {}
            )
            if invalid_value_range is None:
                found_invalid_values = []
            else:
                found_invalid_values = list(
                    unique_values[[v in invalid_value_range for v in unique_values]]
                )

            invalid_labels = [
                existing_labels.get(v, None)
                or f"{v}" + ("(*)" if mark_added_labels else "")
                for v in found_invalid_values
            ]
            if variable.is_numeric_type():
                valid_labels = found_valid_values = []
            else:
                found_valid_values = np.setdiff1d(
                    unique_values,
                    found_invalid_values + variable.invalid_values_as_list(),
                )
                valid_labels = [
                    existing_labels.get(v, None)
                    or f"{v}" + ("(*)" if mark_added_labels else "")
                    for v in found_valid_values
                ]

            if overwrite_existing_information:
                variable.set_values(
                    valid_values=dict(zip(found_valid_values, valid_labels)),
    ...
    def set_variable_types(
        self,
        types: Union[str, list, np.ndarray],
        column_selection: ColumnSelectionTypes = None,
        suppress_check_var_warning=True,
    ):
        ...
        types: str, list or np.ndarray
            ...
            Typical types are 'numerical', 'ordinal', and 'nominal'.
        column_selection : str, int, list, np.ndarray, pd.Series, range, QuestionSelector or None
            If None, all columns are taken into account.
            Otherwise, a subset will be considered. The parameter is interpreted as described in :meth:`Survey.interpret_column_selection`.
        suppress_check_var_warning : bool
        ...
        if not suppress_check_var_warning and not all(
            SurveyVariable.is_valid_type_name(t) for t in types
        ):
            warn(
                "Apparently, you want to use an invalid type. Please try again or cope with the consequences..."
            )
    ...
    def count_number_of_unique_answers(
        self, column_selection: ColumnSelectionTypes = None
    ) -> pd.Series:
        ...
        return (
            self.data[col_sel].nunique()
            if len(col_sel) > 1
            else self.data[col_sel[0]].nunique()
        )
    ...
    def _replace_values_by_labels(self, df, inplace=True):
        ...
        for col in df.columns:
            ...
            df.loc[:, col] = df[col].apply(
                lambda x: (
                    str(f"'{var_info.valid_values.get(v, None)}'" or v for v in x)
                    if isinstance(x, tuple)
                    else var_info.valid_values.get(x, None) or x
                )
            )
        df.rename(
            columns={v.name: (v.label or v.name) for v in self.variables}, inplace=True
        )
        return df
        ...
        for v in self.variables:
            ...
            if (isna := (col_data.isna() | (col_data == "nan"))).any():
                if is_numeric_dtype(self.data.dtypes[v.name]):
                    na = (
                        min(col_data.min(), min(v.invalid_values.keys(), default=0)) - 1
                    )
                else:
                    na, trial = "NaN", 0
