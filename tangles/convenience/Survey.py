...
class Survey:
    ...
    def replace_variable_values(
        ...
        if isinstance(mapping, dict) and len(mapping) > 0:
            target_type_is_number = isinstance(
                next(iter(mapping.values())), numbers.Number
            )
        ...
        for c in column_selection:
            ...
            if (
                len(
                    ...
                    - (set(var.valid_values.keys()) | set(var.invalid_values.keys()))
                )
                > 0
            ):
                warn(
                    "Variable takes values not in the lists: We treat them as valid values..."
                )
                var.add_values(
                    valid_values=zip(
            ...
            if len(unmapped_values) > 0:
                ...
                min_target_value = (
                    min(
                        (
                            min(mapping.values(), default=0)
                            if target_type_is_number
                            else min((int(v) for v in mapping.values()), default=0)
                        ),
                        0,
                    )
                    - 1
                ...
                max_target_value = (
                    max(
                        (
                            max(mapping.values(), default=0)
                            if target_type_is_number
                            else max((int(v) for v in mapping.values()), default=0)
                        ),
                        0,
                    )
                    + 1
    ...
    def guess_missing_variable_types(
        self,
        column_selection: ColumnSelectionTypes = None,
    ...
    def guess_missing_variable_ranges(
        self,
        column_selection: ColumnSelectionTypes = None,
