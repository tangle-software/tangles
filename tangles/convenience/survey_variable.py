...
class UnionOfIntervals:
    ...
    def __init__(
        self, min_=0, max_=0, min_incl=False, max_incl=False, prev=None, next_=None
    ):
        self.min, self.max, self.min_incl, self.max_incl = (
            min_,
            max_,
            min_incl,
            max_incl,
...
class SurveyVariable(ABC):
    ...
    def replace_values(
        self, mapping: dict, combine_labels_if_not_injective: bool = True
    ):
        ...
        if combine_labels_if_not_injective:
            all_values = self.all_values_as_list()
            complete_mapping = dict(zip(all_values, all_values)) | mapping
            if len(complete_mapping.keys()) > len(set(complete_mapping.values())):
                inverse_images = defaultdict(list)
                for x, y in complete_mapping.items():
                    inverse_images[y].append(x)
                complete_value_dict = self.valid_values | self.invalid_values
                for y, x in inverse_images.items():
                    if len(x) > 1:
                        new_label = " | ".join(complete_value_dict[v] for v in x)
                        for v in x:
                            if v in self.valid_values:
                                self.valid_values[v] = new_label
                            elif v in self.invalid_values:
                                self.invalid_values[v] = new_label
    ...
    def is_allowed_operation(self, op):
        if (cl := self.value_class()) is None:
            return False
...
class SurveyVariableValues:
    r"""This class manages the interaction of variables (and their lists of values) and features (or separations).
...
class OrdinalVariableValues(NominalVariableValues):
    ...
    def _update_possible_values_impl(self, sep_metadata: Tuple):
        if sep_metadata[1] in NominalVariableValues.allowed_operations:
            super()._update_possible_values_impl(sep_metadata)
            return
