...
from tangles.search import TangleSearchWidget
...
from tangles.convenience.convenience_functions import (
    search_tangles,
    search_tangles_uncrossed,
)
from tangles.convenience.survey_feature_factory import (
...
from tangles.convenience.convenience_orders import (
...
from tangles.convenience.survey_variable import SurveyVariableValues
from tangles.search.progress import (
    TangleSearchProgressType,
...
class SurveyTangles:
    ...
    @staticmethod
    def load(file) -> "SurveyTangles":
        """Load a :class:`SurveyTangles` from files.

        Parameters
        ----------
        file : str
            A file where the data is stored

        Returns
        ----------
        :class:`SurveyTangles`
            the loaded SurveyTangles object
        """
        with open(file, "rb") as f:
            survey_tangles = pickle.load(f)
        assert isinstance(survey_tangles, SurveyTangles)
        return survey_tangles
    ...
    def search(
        ...
        survey: Survey,
        agreement: int,
        features_or_separations: Optional[np.ndarray] = None,
        metadata: Optional[Union[np.ndarray, list]] = None,
        order: Optional[
            Union[list, np.ndarray, SetSeparationOrderFunction, str]
        ] = None,
        ...
        if features_or_separations is None:
            if not feature_factory:
                feature_factory = SimpleSurveyFeatureFactoryMissingValuesBothSides(
                    survey
                )
            if progress_callback:
                progress_callback(
                    PROGRESS_TYPE_SOMETHING_STARTING, info="creating features..."
                )
            features_or_separations, metadata = feature_factory.create_features()
            if progress_callback:
                progress_callback(
                    PROGRESS_TYPE_SOMETHING_FINISHED,
                    info="creating features... finished",
                )
        ...
        if isinstance(order, str):
            if not order_works_on_features(order):
                if tangles.similarity_matrix is None:
                    warn(
                        "Your order needs a similarity matrix. We create one for you :-) [sorry, this may take a while...]"
                    )
                    if progress_callback:
                        progress_callback(
                            PROGRESS_TYPE_SOMETHING_STARTING,
        ...
        tangles.initialize_search(
            agreement=agreement,
            features_and_metadata=(features_or_separations, metadata),
            uncrossing=uncross,
        )
        return tangles
    ...
    def initialize_search(
        self,
        agreement: int,
        features_and_metadata: Union[
            SetSeparationSystemBase, Tuple[np.ndarray, Union[np.ndarray, None]]
        ],
        uncrossing: bool = False,
    ):
        ...
        features_and_metadata : SetSeparationSystemBase or np.ndarray
            Either a tuple containing a matrix respresentation of the features as well as the metadata
            or an object of type `SetSeparationSystemBase`
        uncrossing : bool
            If True, uncross the features (or separations) that distinguish at least two tangles.
        """

        if isinstance(features_and_metadata, Tuple):
            features, metadata = features_and_metadata
            if metadata is None:
                warn(
                    "SurveyTangles.search: feature meta data missing: some functionality might not be available"
                )
            elif (e := self._check_metadata(metadata)) is not None:
                warn(
                    "Your metadata might not have the right format to use the full functionality of the class SurveyTangles. "
                    "You should consider using a lower level interface."
                    f"Error: {e}"
                )
        else:
            features, metadata = features_and_metadata, None
        ...
        if uncrossing:
            if self.order is None or not isinstance(self.order, Callable):
                raise ValueError(
                    "We need a submodular order function to be able to uncross!"
                )
            self.sweep = search_tangles_uncrossed(
                features,
                agreement,
                order_func=self.order,
                sep_metadata=metadata,
                progress_callback=self.progress_callback,
            )
        else:
            self.sweep = search_tangles(
                features,
                agreement,
                order=self.order,
                sep_metadata=metadata,
    ...
    def specified_features(self, only_original_features: bool = True) -> np.ndarray:
    ...
    def tangle_matrix(
        ...
        remove_prefixing_tangles: bool = True,
        ...
        if (
            ...
            and len(tangle_mat.shape) > 2
            and tangle_mat.shape[0] > 1
        ):
        ...
        if remove_prefixing_tangles:
            ...
            tangle_mat = tangle_mat[
                ~((s & (nonz[:, np.newaxis] < nonz[np.newaxis, :])).any(axis=1)), :
            ]
        return (
            (
                tangle_mat,
                [
                    m.tail_as_list()
                    for m in self.feature_system.separation_metadata(
                        self.sweep.original_feature_ids[: tangle_mat.shape[1]]
                    )
                ],
            )
            if return_metadata
    ...
    def change_agreement(self, agreement: int, force_tree_update: bool = False):
        ...
        if agreement > self.valid_agreement_lower_bound:
            ...
            self.sweep.lower_agreement(
                agreement, progress_callback=self.progress_callback
            )
            self.agreement = agreement
            ...
            warn(
                "Agreement lower than or equal to current valid agreement lower bound. We have to update the tree: you can enforce an update of the tangle tree by setting <force_tree_update> = True"
            )
            return

    def ordered_metadata(
        self, only_original_seps: bool = False, insert_labels: bool = False
    ) -> list:
        ...
        sep_ids = (
            self.sweep.original_feature_ids
        ...
        if insert_labels:
            ...
            for mlist in meta_data:
                for m in mlist:
                    if m.type == "custom":
                        m.info = (
                            variables[m.info[0]].label or m.info[0],
                            m.info[1],
                            variables[m.info[0]].valid_values.get(m.info[2])
                            or m.info[2],
                        )
        return meta_data
    ...
    def typical_answers(
        self,
        only_affected_questions: bool = True,
        column_selection: ColumnSelectionTypes = None,
        insert_labels: bool = True,
        extract_const_answers: bool = False,
        only_max_level_tangles: bool = False,
    ) -> Union[pd.DataFrame, None, Tuple[pd.DataFrame, pd.DataFrame]]:
        ...
        if tangle_mat.shape[1] == 0:
            warn(
                "no tangle contains the original separations, please try something different"
            )
            return (None, None) if extract_const_answers else None

        if only_max_level_tangles:
            tangle_mat = tangle_mat[tangle_mat[:, -1] != 0, :]

        sep_ids = self.sweep.original_feature_ids[: tangle_mat.shape[1]]

        return self._create_answer_matrix(
            tangle_mat,
            sep_ids,
            only_affected_questions=only_affected_questions,
            column_selection=column_selection,
            insert_labels=insert_labels,
            extract_const_answers=extract_const_answers,
        )
    ...
    def explain_feature(
        ...
        specification: int,
        insert_labels: bool = True,
        extract_const_answers: bool = True,
    ) -> Union[pd.DataFrame, Tuple[pd.DataFrame, pd.DataFrame]]:
        """Create a dataframe containing information about a feature specification.
        If the feature is a compound feature, i.e. a corner of other features, the resulting data frame contains the logical formula describing the feature in `Disjunctive Normal Form`.
        Every row of the dataframe describes a conjunction of features, the overall formula then is a logical or of all rows. In other words,
        respondents belong to the feature, if they answer the questions according to the answers given by at least one of the rows.
        ...
        insert_labels : bool
            ...
            Often the label is the question text.
        extract_const_answers : bool
            If True, two dataframes are returned. This parameter only makes sense for compound features (corners),
            in this case, one of the data frames contains the literals that are part of every conjunction in the DNF of the description

        Returns
        -------
        pandas.DataFrame or tuple (DataFrame, DataFrame)
            A dataframe containing a DNF representation of a logical formula describing the given specified feature.
            If the argument `extract_const_answers` is True, a second dataframe is returned containing the literals that are equal in every conjunction of the DNF.
        """

        meta = self.feature_system.feature_metadata(feature_id)
        if meta.type == "custom":
            var = self.survey.variables[meta.info[0]]
            var_values = var.create_values()
            var_values.update_values_for_specification(
                specification, meta.info, meta.orientation
            )
            df = pd.DataFrame(
                data=[[var_values.possible_values_representation(True)]],
                columns=[var.label],
            )
            return (df, None) if extract_const_answers else df
        sep_ids = self.sweep.original_feature_ids
        meta_data_list = self.feature_system.separation_metadata(sep_ids)
        dnf = self.feature_system.metadata_matrix(
            (feature_id, specification),
            data_list=[m.info for m in meta_data_list],
            normal_form="disjunctive",
        ).T
        relevant_col = (dnf != 0).any(axis=0)
        return self._create_answer_matrix(
            dnf[:, relevant_col],
            [_id for i, _id in enumerate(sep_ids) if relevant_col[i]],
            insert_labels=insert_labels,
            extract_const_answers=extract_const_answers,
        )

    def save(self, file_name: str, append_date_to_name=False):
        warn(
            "Save functionality is very rudimentary, it might be a problem to load the stored file after the library changes..."
        )
    ...
    def _create_answer_matrix(
        ...
        metadata = [
            [
                m
                for m in self.sweep.sep_sys.separation_metadata(sep_id).tail_as_gen()
                if m.type == "custom"
            ]
        ...
        answers = pd.DataFrame(
            index=range(orientation_matrix.shape[0]), columns=selected_cols, data=None
        )
        for tangle_idx in range(orientation_matrix.shape[0]):
            var_values = self._answers_for_orientation(
                orientation_matrix[tangle_idx, :], metadata, included_vars
            )
            for v in var_values:
                value_rep = v.possible_values_representation(insert_labels)
                answers.loc[tangle_idx, v.var.name] = value_rep

        if insert_labels:
            answers.rename(
                columns={
                    v.name: (v.label or v.name) + f"[{v.name}]"
                    for v in self.survey.variables
                },
                inplace=True,
            )

        if extract_const_answers:
            const = answers.nunique() == 1
            const_answers = pd.DataFrame(answers.loc[0, const]).T
            answers = answers.loc[:, ~const]
            return answers, const_answers
        return answers

    def _answers_for_orientation(self, orientation, metadata_list, included_vars=None):
        if included_vars:
            answers = {
                q.name: q.create_values()
        ...
        for metadata, ori in zip(metadata_list, orientation):
            ...
            for m in metadata:
                if m.info[0] not in answers:
                    continue
                answers[m.info[0]].update_values_for_specification(
                    ori, m.info, m.orientation
                )
    ...
    def _normalize_meta_data(self):
        for m in self.sweep.sep_sys.separation_metadata():
            while m:
                if (
                    m.type == "custom" and m.orientation < 0
                ):  # TODO: what about corners?
                    m.orientation, m.info = 1, (
                        m.info[0],
                        SurveyVariableValues.invert_op(m.info[1]),
                        m.info[2],
                    )
                m = m.next
