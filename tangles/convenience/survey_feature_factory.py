...
# TODO: refactor lower level factory functions!
...
class SurveyFeatureFactory(ABC):
    ...
    @abstractmethod
    def create_features_for_single_col(
        self, variable: SurveyVariable, col_data: pd.Series
    ...
    def create_features(
        ...
        tuple[np.ndarray, np.ndarray]
            ...
            In this matrix, every entry is a tuple ``(column_name, operation, value)``. For example, the tuple
            ``('feature_name', '>=', 8)`` describes the feature (or separation) 'feature_name' that splits the column
            into a group that answered less than 8 and one that answered at least 8.
        """
        ...
        for c in columns:
            ...
            s, meta = self.create_features_for_single_col(
                self.survey.variables[c], self.survey.data[c]
            )
...
class SurveyFeatureFactoryDecorator(SurveyFeatureFactory, ABC):
    ...
    def __init__(
        self,
        survey: Survey,
        embedded_feature_factory: Optional[SurveyFeatureFactory] = None,
    ):
        super().__init__(survey)
        self.embedded_feature_factory = (
            embedded_feature_factory or SimpleSurveyFeatureFactory(survey)
        )
...
class SimpleSurveyFeatureFactory(SurveyFeatureFactory):
    ...
    create_feature_func = Callable[
        [pd.Series, Union[list, np.ndarray, pd.Series]], Tuple[np.ndarray, np.ndarray]
    ]
    ...
    def __init__(self, survey: Survey):
        ...
        def num_func(c, i):
            return create_features_split_regular_ge(
                c,
                num_bins=SurveyFeatureFactory.default_numeric_var_num_bins,
                max_num_values_for_extensive_seps=50,
                invalid_values=i,
            )

        self.numvar_func: SimpleSurveyFeatureFactory.create_feature_func = num_func
        self.ordvar_func: SimpleSurveyFeatureFactory.create_feature_func = (
            feature_factory_all_splits_ge
        )
        self.nomvar_func: SimpleSurveyFeatureFactory.create_feature_func = (
    ...
    def create_features_for_single_col(
        ...
        if variable.type in SurveyVariable.ordinal_types:
            return self.ordvar_func(col_data, invalid)
        if variable.type in SurveyVariable.nominal_types:
            return self.nomvar_func(col_data, invalid)
        warn("cannot create features for unknown variable types")
        return None


class SimpleSurveyFeatureFactoryMissingValuesOwnFeatures(SurveyFeatureFactoryDecorator):
    ...
    def create_features_for_single_col(
        ...
        seps, meta = self.embedded_feature_factory.create_features_for_single_col(
            variable=variable, col_data=col_data
        )
        if variable.invalid_values:
            ...
            new_meta[0] = (
                "in",
                (
                    UnionOfIntervals.create_with_isolated_points(invalid_values_list)
                    if variable.is_numeric_type()
                    else invalid_values_list
                ),
            )
...
class SimpleSurveyFeatureFactoryMissingValuesBothSides(SurveyFeatureFactoryDecorator):
    """A simple survey factory that takes missing and invalid values into account.
...
class SimpleSurveyFeatureFactoryMissingValuesImplicit(SimpleSurveyFeatureFactory):
    """A simple survey factory that takes missing and invalid values into account.
    ...
    def create_features_for_single_col(
        ...
        invalid = variable.invalid_values_as_list()
        if variable.type in SurveyVariable.numeric_types:
            return create_features_split_regular_bins(
                col_data, num_bins=self.numeric_var_num_bins, invalid_values=invalid
            )
        if variable.type in SurveyVariable.ordinal_types:
...
class SurveyFeatureFactoryCherryPicker(SurveyFeatureFactoryDecorator):
    ...
    def __init__(self, survey: Survey, default: Optional[SurveyFeatureFactory] = None):
        super().__init__(
            survey, default or SimpleSurveyFeatureFactoryMissingValuesBothSides(survey)
        )
        self.cherries = {}
    ...
    def create_features_for_single_col(
        ...
        return self.embedded_feature_factory.create_features_for_single_col(
            variable, col_data
        )
...
def binary_unique_value_features(
    single_col_data: Union[pd.Series, np.ndarray],
    unique_values: Union[list, np.ndarray],
    op: str = "==",
) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    if len(unique_values) != 2:
        raise ValueError(
            "_binary_unique_value_features: variable is supposed to take exactly two values"
        )
...
def create_features_all_unique_values(
    single_col_data: Union[pd.Series, np.ndarray],
    unique_values: Union[list, np.ndarray, None] = None,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    if unique_values is None:
        unique_values = single_col_data.unique()
    if len(unique_values) == 2:
...
def create_features_split_regular_ge(
    single_col_data: Union[pd.Series, np.ndarray],
    num_bins: int = 5,
    max_num_values_for_extensive_seps: Optional[int] = 50,
    invalid_values: Optional[Union[list, np.ndarray]] = None,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    unique_values = (
        np.setdiff1d(single_col_data.unique(), invalid_values)
        if invalid_values
        else single_col_data.unique()
    )
    if (l := len(unique_values)) <= 2:
        return binary_unique_value_features(single_col_data, unique_values, op="==")
    if max_num_values_for_extensive_seps and l <= max_num_values_for_extensive_seps:
        return feature_factory_all_splits_ge(single_col_data, invalid_values)
    ...
    for i, v in enumerate(
        (min(unique_values) * (1 - splits) + max(unique_values) * splits)
    ):
...
def create_features_split_regular_bins(
    single_col_data: Union[pd.Series, np.ndarray],
    num_bins: int = 5,
    invalid_values: Optional[Union[list, np.ndarray]] = None,
) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for i in range(num_bins):
        ...
        feat[
            (single_col_data >= lo)
            & ((single_col_data < hi) if i < num_bins - 1 else (single_col_data <= hi)),
            i,
        ] = 1
...
def feature_factory_all_splits_ge(
    single_col_data: Union[pd.Series, np.ndarray],
    invalid_values: Optional[Union[list, np.ndarray]] = None,
) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def nominalvar_features_all_cats(
    single_col_data: Union[pd.Series, np.ndarray],
    invalid_values: Optional[Union[list, np.ndarray]] = None,
) -> Tuple[np.ndarray, np.ndarray]:
    ...
    return create_features_all_unique_values(
        single_col_data, np.setdiff1d(single_col_data.unique(), invalid_values)
    )
...
def feats2seps_invalids_to_both_sides(create_feature_func):
    ...
    def func(
        single_col_data: Union[pd.Series, np.ndarray],
        invalid_values: Union[list, np.ndarray, pd.Series],
    ):
