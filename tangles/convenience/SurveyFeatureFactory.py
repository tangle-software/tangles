...
class SimpleSurveyFeatureFactory(SurveyFeatureFactory):
    ...
    def __init__(self, survey: Survey):
        super().__init__(survey)
        num_func = lambda c, i: numericvar_features_split_regular_ge(
            c,
...
class SimpleSurveyFeatureFactory_MissingValuesImplicit(SimpleSurveyFeatureFactory):
    ...
    def create_features_for_single_col(
        ...
        invalid = variable.invalid_values_as_list()
        if variable.type in SurveyVariable.numeric_types:
            return numericvar_features_inside_regular_bins(
...
class SurveyFeatureFactory_CherryPicker(SurveyFeatureFactoryDecorator):
    ...
    def __init__(self, survey: Survey, default: Optional[SurveyFeatureFactory] = None):
        super().__init__(
            survey, default or SimpleSurveyFeatureFactory_MissingValuesBothSides(survey)
        )
        self.cherries = {}
...
def simple_unique_value_features(
    single_col_data: Union[pd.Series, np.ndarray],
    unique_values: Union[list, np.ndarray],
) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def numericvar_features_split_regular_ge(
    single_col_data: Union[pd.Series, np.ndarray],
    num_bins: int = 5,
...
def numericvar_features_inside_regular_bins(
    single_col_data: Union[pd.Series, np.ndarray],
    num_bins: int = 5,
    invalid_values: Optional[Union[list, np.ndarray]] = None,
) -> Tuple[np.ndarray, np.ndarray]:
...
def extensive_numericvar_features(
    single_col_data: Union[pd.Series, np.ndarray],
    invalid_values: Optional[Union[list, np.ndarray]] = None,
) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def ordinalvar_features_ge_all_splits(
    single_col_data: Union[pd.Series, np.ndarray],
    invalid_values: Optional[Union[list, np.ndarray]] = None,
) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def nominalvar_features_all_cats(
    ...
    return simple_unique_value_features(
