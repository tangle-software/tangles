...
from tangles.convenience.SurveyFeatureFactory import (
    ...
    SimpleSurveyFeatureFactory_MissingValuesBothSides,
)
...
class SurveyTangles:
    ...
    def search(
        ...
        if features_or_separations is None:
            if not feature_factory:
                feature_factory = SimpleSurveyFeatureFactory_MissingValuesBothSides(
                    survey
                )
        ...
        tangles.initialize_search(
            agreement=agreement,
            features_or_separations=features_or_separations,
            metadata=metadata,
            uncrossing=uncross,
        )
        return tangles
    ...
    def sepcified_features(self, only_original_features: bool = True) -> np.ndarray:
        ...
        if only_original_features:
            oriented_seps = oriented_seps[
                np.isin(oriented_seps, self.sweep.original_sep_ids)
            ]
    ...
    def tangle_matrix(
        ...
        return (
            (
                tangle_mat,
                self.feature_system.separation_metadata(
                    self.sweep.original_sep_ids[: tangle_mat.shape[1]]
                ),
            )
            if return_metadata
            else tangle_mat
    ...
    def ordered_metadata(
        ...
        sep_ids = (
            self.sweep.original_sep_ids
            if only_original_seps
            else self.sweep.all_oriented_sep_ids
        )
        meta_data = deepcopy(
    ...
    def typical_answers(
        ...
        remove_incomplete_tangles: bool = False,
        ...
        typical_answers = pd.DataFrame(
            index=range(tangle_mat.shape[0]), columns=selected_cols, data=None
        )
        for tangle_idx in range(tangle_mat.shape[0]):
            var_values = self._answers_for_orientation(
                tangle_mat[tangle_idx, :], metadata, included_vars
            )
            for v in var_values:
