...
def tangle_score(
    ...
    if len(feat_ids) != tangle_matrix.shape[1]:
        raise ValueError(
            """Number of feature ids must be equal to the number of columns in tangle_matrix. If tangle_matrix
                         and feature ids come directly from the TangleSearchTree, use feat_ids[:tangle_matrix.shape[1]]."""
        )
    scores = (
        feat_sys[feat_ids] @ tangle_matrix.astype(float).T
        if tangle_matrix.dtype != float
        else feat_sys[feat_ids] @ tangle_matrix.T
    )
