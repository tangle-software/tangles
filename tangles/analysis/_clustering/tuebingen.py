...
def hard_clustering_tuebingen(
    tangle_matrix: np.ndarray, feat_ids: np.ndarray, feat_sys
) -> np.ndarray:
    """
    ...
    def prob_calculator(_, distinguisher_idx: int, feat_ids: np.ndarray) -> np.ndarray:
        return ((feat_sys[feat_ids[distinguisher_idx]] + 1) / 2).astype(int)[
            np.newaxis, :
        ]

    return _clustering_tuebingen(
        tangle_matrix, feat_ids, feat_sys.datasize, prob_calculator
    ).T


def soft_clustering_tuebingen(
    tangle_matrix: np.ndarray, feat_ids: np.ndarray, feat_sys
) -> np.ndarray:
    """
    ...
    def prob_calculator(
        tangle_matrix: np.ndarray, distinguisher_idx: int, feat_ids: np.ndarray
    ) -> np.ndarray:
        ...
        probability_split = (
            ((feat_sys[feat_ids[pos_ids]].astype(float) + 1) / 2).sum(axis=1)
            + ((-feat_sys[feat_ids[neg_ids]].astype(float) + 1) / 2).sum(axis=1)
        ) / (np.sum(pos_ids) + np.sum(neg_ids))
...
def _clustering_tuebingen(
    tangles: np.ndarray,
    feat_ids: np.ndarray,
    num_datapoints: int,
    prob_calculator: Callable,
) -> np.ndarray:
    ...
    left_clustering_matrix = _clustering_tuebingen(
        tangles[left][:, distinguisher_idx:],
        feat_ids[distinguisher_idx:],
        num_datapoints,
        prob_calculator,
    ) * (1 - prob_vector)
    right_clustering_matrix = _clustering_tuebingen(
        tangles[right][:, distinguisher_idx:],
        feat_ids[distinguisher_idx:],
        num_datapoints,
        prob_calculator,
    ) * (prob_vector)
