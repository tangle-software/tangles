...
def build_random(
    min_number_of_nodes: int, height: int, average_agreement_change: int = 3
) -> TangleSearchTree:
...
def _choose_random_extendable_leaf(root: Tangle, max_height: int) -> Tangle:
    ...
    leaves = [
        leaf for leaf in leaves if leaf.agreement > 0 and leaf.level() < max_height
    ]
...
def _extend_node(node: Tangle, average_agreement_change: int) -> tuple[Tangle, Tangle]:
    node.set_left_child(
        Tangle(node.agreement - np.random.randint(2 * average_agreement_change + 1))
    )
