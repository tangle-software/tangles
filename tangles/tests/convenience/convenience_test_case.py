...
class ConvenienceTestCase:
    def __init__(
        self, name: str, seps: np.ndarray, order_func: SetSeparationOrderFunction
    ):
        self._name = name
        self._seps = seps
    ...
    @staticmethod
    def load_test_case_covariance(
        name: str, shift=0
    ):  # shift is needed nearly always if a covariance matrix is used to build a ToT (submodularity!)
        test_npz = np.load(get_test_data_path(f"agreement_test_cases/{name}.npz"))
    ...
    def test_tst(self, agreement: int):
        ...
        assert self._less_original_seps_than_max_number_of_seps(
            sep_sys, agreement, max_number_of_seps=10
        )

        uncrossed_sweep = search_tangles_uncrossed(sep_sys, agreement, self._order_func)
        assert self._tangle_matrix_only_original_seps_parameter(
            uncrossed_sweep, len(sep_sys)
        )

        tangle_matrix = uncrossed_sweep.tree.tangle_matrix(agreement)
        scores = tangle_score(
            tangle_matrix,
            uncrossed_sweep.tree.sep_ids[: tangle_matrix.shape[1]],
            uncrossed_sweep.sep_sys,
            normalize_rows=True,
        )
    ...
    def _less_original_seps_than_max_number_of_seps(
        self, sep_sys, agreement, max_number_of_seps
    ):
        uncrossed_sweep = search_tangles_uncrossed(
            sep_sys, agreement, self._order_func, max_number_of_seps
        )
        return uncrossed_sweep._number_of_initial_seps_added <= max_number_of_seps

    def _tangle_matrix_only_original_seps_parameter(
        self, uncrossed_sweep, number_of_original_seps
    ):
        tangle_matrix = uncrossed_sweep.tangle_matrix(only_initial_seps=True)
    ...
    def _verify_tot_tree_structure(self, tot):
        for edge in tot.edges:
            ...
            assert self._subtrees_contain_all_tot_nodes(
                nodes_in_subtree_1, nodes_in_subtree_2, tot.nodes
            )
    ...
    def _subtrees_contain_all_tot_nodes(
        self, nodes_in_subtree_1, nodes_in_subtree_2, all_tot_nodes
    ):
    ...
    def _verify_tot_splits(self, tot):
        for edge in tot.edges:
            ...
            assert (
                self._two_nodes_from_different_subtrees_orient_sep_of_edge_differently(
                    nodes_in_subtree_1, nodes_in_subtree_2, edge.sep_idx
                )
            )
            assert self._two_nodes_from_same_subtree_orient_sep_of_edge_equally(
                nodes_in_subtree_1, edge.sep_idx
            )
            assert self._two_nodes_from_same_subtree_orient_sep_of_edge_equally(
                nodes_in_subtree_2, edge.sep_idx
            )

    def _edge_sep_idx_points_to_correct_sep_id(self, edge, sep_ids):
        return sep_ids[edge.sep_idx] == edge.sep_id

    def _two_nodes_from_different_subtrees_orient_sep_of_edge_differently(
        self, nodes_in_subtree_1, nodes_in_subtree_2, sep_idx_of_edge
    ):
        for n1, n2 in product(nodes_in_subtree_1, nodes_in_subtree_2):
            if (
                n1.reduced_tangle[sep_idx_of_edge] == 0
                and n2.reduced_tangle[sep_idx_of_edge] == 0
            ):
                continue
            elif (
                n1.reduced_tangle[sep_idx_of_edge] == n2.reduced_tangle[sep_idx_of_edge]
            ):
                return False
        return True

    def _two_nodes_from_same_subtree_orient_sep_of_edge_equally(
        self, nodes_in_subtree, sep_idx_of_edge
    ):
        for n_a, n_b in combinations(nodes_in_subtree, 2):
            if (
                n_a.reduced_tangle[sep_idx_of_edge] == 0
                or n_b.reduced_tangle[sep_idx_of_edge] == 0
            ):
                continue
            elif (
                n_a.reduced_tangle[sep_idx_of_edge]
                != n_b.reduced_tangle[sep_idx_of_edge]
            ):
                return False
        return True
    ...
    def _verify_tot_respects_sep_le_ordering(self, tot, le_func):
        ...
        self._check_correct_le_ordering_to_children(
            node_direction_1, start_edge, le_func
        )
        self._check_correct_le_ordering_to_children(
            node_direction_2, start_edge, le_func
        )
