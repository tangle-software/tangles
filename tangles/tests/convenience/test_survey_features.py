...
def test_survey_separations_simple():
    ...
    assert seps.shape == (
        data.shape[0],
        sum((num_nom_seps, num_ord_seps, num_scale_seps, num_bin_seps)),
    )
    ...
    assert set([m[2] for m in meta[num_nom_seps : num_nom_seps + num_ord_seps]]) == set(
        pd_data["ord"].unique()
    ) - {pd_data["ord"].min()}
    ...
    assert all(
        m[0] == "scale"
        for m in meta[
            num_nom_seps + num_ord_seps : num_nom_seps + num_ord_seps + num_scale_seps
        ]
    )
    assert all(
        m[1] == ">="
    ...
    assert (
        np.maximum(
            -seps[:, num_nom_seps],
            np.maximum.reduce(
                seps[:, num_nom_seps : num_nom_seps + num_ord_seps], axis=1
            ),
        )
        == 1
    ).all()
    assert all(
        (seps[:, i] >= seps[:, i + 1]).all()
        for i in range(num_nom_seps, num_nom_seps + num_ord_seps - 1)
    )

    assert (
        np.maximum(
            -seps[:, num_nom_seps + num_ord_seps],
            np.maximum.reduce(
                seps[
                    :,
                    num_nom_seps
    ...
    assert all(
        (seps[:, i] >= seps[:, i + 1]).all()
        for i in range(
            num_nom_seps + num_ord_seps,
            num_nom_seps + num_ord_seps + num_scale_seps - 1,
        )
    )
    ...
    for invalid in [[], [-100, -200]]:
        for num_valid_values in [2, 5, 10, 100]:
            ...
            for i, m in enumerate(meta):
                assert all(
                    2 * eval(f"p {m[0]} m[1]") - 1 == seps[j, i]
                    for j, p in enumerate(col)
                    if p not in invalid
                )
                assert all(
                    2 * eval(f"p {SurveyVariableValues.invert_op(m[0])} m[1]") - 1
                    == -seps[j, i]
                    for j, p in enumerate(col)
                    if p not in invalid
                )
...
def test_binary_unique_value_features(op):
    ...
    assert meta[0][0] == op
    assert all(
        2 * eval(f"p {op} {meta[0][1]}") - 1 == seps[j, 0] for j, p in enumerate(col)
    )
...
def test_simple_unique_value_features():
    ...
    seps, meta = create_features_all_unique_values(
        single_col_data=col, unique_values=unique_v
    )
    assert seps.shape[1] == meta.shape[0]
    assert seps.shape[1] == len(unique_v)
    for i, m in enumerate(meta):
        assert all(
            2 * eval(f"p {m[0]} m[1]") - 1 == seps[j, i] for j, p in enumerate(col)
        )
...
def test_numericvar_features_split_regular_ge():
    ...
    for num_bins in range(2, 7):
        seps, meta = create_features_split_regular_ge(
            col,
            num_bins=num_bins,
            invalid_values=invalid,
            max_num_values_for_extensive_seps=None,
        )
        ...
        for i in range(seps.shape[1]):
            ...
            assert all(
                2 * (p >= meta[i][1]) - 1 == seps[j, i] for j, p in enumerate(col)
            )
...
def test_numericvar_features_inside_regular_bins():
    ...
    min, max = data.min(), data.max()
    for num_bins in range(2, 7):
        seps, meta = create_features_split_regular_bins(
            col, num_bins=num_bins, invalid_values=invalid
        )
        ...
        for i in range(num_bins):
            ...
            assert all(
                2 * (p in meta[i][1]) - 1 == seps[j, i] for j, p in enumerate(col)
            )
...
def test_ordinalvar_features_ge_all_splits():
    ...
    unique_v = np.unique(data)
    col = pd.Series(data=np.append(data, invalid))
    seps, meta = feature_factory_all_splits_ge(
        single_col_data=col, invalid_values=invalid
    )
    assert seps.shape[1] == meta.shape[0]
    ...
    for i, m in enumerate(meta):
        assert all(
            2 * (p >= unique_v[i + 1]) - 1 == seps[j, i] for j, p in enumerate(col)
        )
...
def test_nominalvar_features_all_cats():
    ...
    unique_v = np.unique(data)
    col = pd.Series(data=np.append(data, invalid))
    seps, meta = nominalvar_features_all_cats(
        single_col_data=col, invalid_values=invalid
    )
    assert seps.shape[1] == meta.shape[0]
    assert seps.shape[1] == len(unique_v)
