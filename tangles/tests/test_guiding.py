    ...
    for i in range(len(path)):
        ...
        if not found_leaf_on_path:
            ...
            path_from_root = path_from_root + [0] * (
                len(test_path) - len(path_from_root)
            )
...
def _test_agreements_along_path(tree: TangleSearchTree, path: np.ndarray):
    ...
    for i in range(len(tangles_along_path)):
        ...
        for agreement in range(
            agreement_must_be_greater_than + 1, agreement_must_be_at_most + 1
        ):
            assert (
                get_tangle_by_path(tree, path, min_agreement=agreement)
                == tangles_along_path[i]
            )
