...
from ._test_data.generate_tangle_matrix import (
    generate_small_example_with_splitting,
...
def _test_small_example_hard_clustering():
    ...
    clustering = hard_clustering_tuebingen(
        tangle_matrix, np.arange(len(feat_sys)), feat_sys
    )
...
def _test_not_only_leafs_hard_clustering():
    ...
    intended_clustering = load(
        "big_5_hard_clustering_with_splitting.csv", dtype=float
    ).T
    assert np.all(clustering == intended_clustering)
...
def _test_small_example_soft_clustering():
    tangle_matrix, feat_sys = generate_small_example()
    clustering = soft_clustering_tuebingen(
        tangle_matrix, np.arange(len(feat_sys)), feat_sys
    )
