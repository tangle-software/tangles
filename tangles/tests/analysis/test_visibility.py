...
def test_visibility():
    ...
    assert np.all(
        visibility(tangle_matrix, column_orders=np.arange(tangle_matrix.shape[1]))
        == np.array([4, 1, 3, 3, 3, 1, 2, 3, 3])
    )
...
def _complexity_not_just_leafs():
    _, tangle_matrix = _generate_basic_example_tangle_matrix()
    assert np.all(
        complexity_levels(tangle_matrix) == np.array([1, 6, 6, 6, 6, 0, 1, 3, 3])
    )


def _complexity_just_one_tangle():
    assert np.all(
        complexity_levels(np.array([1, -1, 1, -1, 1, -1])[np.newaxis, :])
        == np.array([0])
    )


def _generate_from_intended_coherence(
    intended_coherence: np.ndarray, num_feats: int = 10
):
