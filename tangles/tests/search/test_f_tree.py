...
def test_empty_forbidden_set(tst, agreement_func, ftree_only_root):
    new_ftree = create_ftree(tst, agreement_func, 11)
    new_ftree.make_irreducible()
...
def test_forced_reduces_agreement_error(tst, agreement_func):
    ...
    try:
        _ = create_ftree(tst, agreement_func, 2)
    except:
    ...
    return _build_agreement_func(
        [
            ...
            (
                set([(1, -1)]),
                0,
            ),  # ensuring that 0 is nowhere necessary on the right side
            (set([(0, 1), (3, 1)]), 0),  # only 0 and 3 would be sufficient
            (
                set([(0, 1), (3, -1)]),
                0,
            ),  # but since 0 is removed first this is not realized
            (set([(1, 1), (2, -1)]), 0),
            ...
            (
                set([(1, 1), (3, 1)]),
                0,
            ),  # last leaf, 1 is only necessary if 0 is deleted
        ]
    )
...
def _build_agreement_func(
    agreement_value_subsets: list[tuple[set, int]], max_value: int = 10
) -> AgreementFunc:
