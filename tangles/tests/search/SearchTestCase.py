...
class SimpleTSTestCase:
    ...
    def build(name: str) -> "SimpleTSTestCase":
        ...
        for i in sep_info[0]:
            if search.append_separation(i, min(agreements)) == 0:
                break
        if not is_tst_valid(
            search.tree, feat_sys.is_le, agreement_func(feat_sys), min(agreements)
        ):
            return None
        return SimpleTSTestCase(
            name=name, feats=feats, agreements=agreements, tree=search.tree
        )
    ...
    def load(name: str) -> "SimpleTSTestCase":
        ...
        with open(
            get_test_data_path(f"tst_data/{name}/parent_index.pickle"), "rb"
        ) as f:
        ...
        with open(get_test_data_path(f"tst_data/{name}/cores.pickle"), "rb") as f:
            cores = pickle.load(f)
        with open(
            get_test_data_path(f"tst_data/{name}/tangle_agreements.pickle"), "rb"
        ) as f:
        ...
        if not is_tst_valid(
            tree, feat_sys.is_le, agreement_func(feat_sys), min(agreements)
        ):
    ...
    def save(self) -> "SimpleTSTestCase":
        ...
        with open(
            get_test_data_path(f"tst_data/{self._name}/agreements.pickle"), "wb"
        ) as f:
        ...
        with open(
            get_test_data_path(f"tst_data/{self._name}/parent_index.pickle"), "wb"
        ) as f:
            pickle.dump(parent_index, f)
        with open(
            get_test_data_path(f"tst_data/{self._name}/which_child.pickle"), "wb"
        ) as f:
            pickle.dump(which_child, f)
        with open(get_test_data_path(f"tst_data/{self._name}/cores.pickle"), "wb") as f:
            pickle.dump(cores, f)
        with open(
            get_test_data_path(f"tst_data/{self._name}/tangle_agreements.pickle"), "wb"
        ) as f:
            pickle.dump(tangle_agreements, f)
        return self

    def run_append(self, agreement: int) -> bool:
        feat_sys, sep_info = SetSeparationSystem.with_array(
            self._feats, return_sep_info=True
        )
        search = TangleSweep(agreement_func(feat_sys), le_func=feat_sys.is_le)
        for i in sep_info[0]:
    ...
    def _sweep_test(self):
        feat_sys, [sep_ids, _] = SetSeparationSystem.with_array(
            self._feats, return_sep_info=True
        )
    ...
    def _sweep_append_test(self):
        ...
        for sep_id in range(num_seps):
            sweep.append_separation(
                sep_ids[sep_id], agreement_lower_bound=min_test_agreement
            )
        assert sweep.tree.is_subtree_of(self._tree)
    ...
    def _sweep_insert_test(self):
        ...
        for i in range(1, num_seps, 2):
            sweep.append_separation(
                sep_ids[i], agreement_lower_bound=min_test_agreement
            )
        for i in range(0, num_seps, 2):
            sweep.insert_separation(
                sep_ids[i], sep_ids[i], agreement_lower_bound=min_test_agreement
            )
        assert sweep.tree.is_subtree_of(self._tree)
    ...
    def _sweep_greedy_test(self):
        ...
        assert is_tst_valid(
            sweep.tree, feat_sys.is_le, agreement_func(feat_sys), min_test_agreement
        )
