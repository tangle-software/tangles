import pytest
import numpy as np
from tangles.util.preprocessing import standardize, balance, normalize_length


@pytest.fixture
def standardize_data() -> np.ndarray:
    return np.array([[1, 656], [3, 100]])
...
def test_standardize_axis_0(
    standardize_data: np.ndarray, standardized_result: np.ndarray
):
    assert np.all(standardize(standardize_data, axis=0) == standardized_result)


def test_standardize_axis_1(
    standardize_data: np.ndarray, standardized_result: np.ndarray
):
...
def test_standardize_removes_equal_values(
    standardize_data_std_0: np.ndarray, standardized_result_std_0: np.ndarray
):
    assert np.all(
        standardize(standardize_data_std_0.T, axis=1) == standardized_result_std_0.T
    )
...
def normalize_result() -> np.ndarray:
    return np.array([[0, 1 / np.sqrt(2)], [1, 1 / np.sqrt(2)]], dtype=float)
