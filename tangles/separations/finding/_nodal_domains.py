...
def _nodal_domains(
    A: np.ndarray, f: np.ndarray, domain_type: str = "weak", eps: float = 1e-10
) -> np.ndarray:
    if domain_type == "weak":
        sel_n = np.nonzero(f < eps)[0]
        sel_p = np.nonzero(f > -eps)[0]
    elif domain_type == "strong":
        sel_n = np.nonzero(f < -eps)[0]
        sel_p = np.nonzero(f > eps)[0]
    else:
        sel_n = np.nonzero(f <= eps)[0]
...
def nodal_domains(
    A: np.ndarray, U: np.ndarray, domain_type: str = "weak", eps: float = 1e-10
) -> np.ndarray:
    ...
    vertices the function assigns a positive value to (see the `domain_type` parameter description for more details).
    ...
    for i in range(U_matrix.shape[1]):
        nds = _nodal_domains(
            A, U_matrix[:, i], domain_type=domain_type, eps=eps
        )  # what was this 'flat' in here? This returns an iterator! this can be fatal!
        if nds.shape[1] == 2:
