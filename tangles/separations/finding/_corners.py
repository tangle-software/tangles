...
def _add_corner_one_orientation(
    features: np.ndarray, orientation_new: int, orientation_old: int
):
    sep_1 = features * orientation_new
    sep_2 = features * orientation_old
    corners = np.ones(
        (features.shape[0], features.shape[1], features.shape[1]), dtype=np.int8
    )
