...
def spectral_features(
    L: Union[sparse.spmatrix, np.ndarray], k: int, return_eigenvectors=False
    ...
    if sparse.issparse(L):
        _, U = sparse_linalg.eigsh(
            L, k=k, which="LM", sigma=1e-12, v0=np.ones(L.shape[0])
        )
    else:
...
def spectral_features_splitted(
    ...
    if check_connected:
        cc, comp_labels = csg.connected_components(
            L < 0, directed=False, return_labels=True
        )
        if cc == 1:
            ...
            if n_seps < k:
                ...
                for i, s in enumerate(seps_per_component):
                    S_c = spectral_features(
                        L[component_indicators[:, i], :][:, component_indicators[:, i]],
                        s + 1,
                    )
