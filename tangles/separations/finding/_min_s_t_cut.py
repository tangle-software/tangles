...
def _augmented_graph_S_T(A: sparse.csr_matrix, S, T):
    ...
    data = np.r_[
        np.insert(A.data, insert_idcs, int(1e10)),
        np.ones(len(ST), dtype=int) * int(1e10),
    ]
    indptr = np.r_[
        A.indptr, A.indices.shape[0] + len(S), A.indices.shape[0] + len(S) + len(T)
    ]
    for i in ST + 1:
        indptr[i:] += 1
...
def min_S_T_cut(A: sparse.csr_array, S: Union[set, list], T: Union[set, list]):
    ...
    small_side = csg.breadth_first_order(
        residual_network, A.shape[0], return_predecessors=False
    )
