...
class SetSeparationSystemOrderFunc:
    ...
    def _update_order_cache(self):
        ...
        if beginning < num_seps:
            ...
            np.around(
                self.order_func(new_seps), 10, out=new_orders[1, :]
            )  # this seems to fix the numerical problem. but is it always correct?
            self._orders = np.c_[
                self._orders, new_orders
            ]  # we do all this 'injectivity stuff' to be always correct, not 'nearly always' correct - the latter could be much easier..
            self._args = np.lexsort(self._orders)
            self._order_finetuning(beginning)
            self._args_inv = self._args.argsort()

    def _order_finetuning(
        self, first_new_idx
    ):  # we should rethink this, maybe there is a faster solution or a better implementation?
        sorted_orders = self._orders[:, self._args]
        sorted_orders_jumps = 1 + np.flatnonzero(
            (sorted_orders[:, 1:] != sorted_orders[:, :-1]).any(axis=0)
        )
        range_start = 0
        for jump in sorted_orders_jumps:
            if jump - range_start > 1 and np.any(
                self._args[range_start:jump] >= first_new_idx
            ):
                seps = self.sep_sys[self._args[range_start:jump]]
                seps *= seps[0:1, :]
                self._args[range_start:jump] = self._args[range_start:jump][
                    np.lexsort(seps)[::-1]
                ]
            range_start = jump
    ...
    def get_insertion_index(self, sorted_sep_id_list, sep_id_to_insert):
        self._update_order_cache()
        return bisect_left(
            self._args_inv[sorted_sep_id_list], self._args_inv[sep_id_to_insert]
        )
