...
class SetSeparationSystemBase(ABC):
    ...
    @classmethod
    def with_array(
        cls, S: np.ndarray, return_sep_info: bool = False, metadata=None
    ...
    def with_sparse_array(
        cls, S, return_sep_info: bool = False, metadata=None
    ) -> Union[
        ...
        ids, orientations = np.empty(S.shape[1], dtype=int), np.empty(
            S.shape[1], dtype=np.int8
        )
    ...
    def separation_metadata(
        self, sep_ids: Union[int, list, np.ndarray, None] = None
