...
def process_module(pruner, name, mod):
    ...
    functions = pruner.prune([
        ...
        for name, func in sorted(inspect.getmembers(mod, predicate=inspect.isfunction))
        if func.__module__.startswith(mod.__name__)
    ])
    sub_mods = pruner.prune([
        process_module(pruner, name, sub_mod)
        for name, sub_mod in sorted(inspect.getmembers(mod, predicate=inspect.ismodule))
